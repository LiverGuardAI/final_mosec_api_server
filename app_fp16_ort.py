...
class DICOMSegmentationWorker(Worker):
    ...
    def run_inference(self, nifti_path: str, idx: int) -> str:
        """Run nnU-Net ONNX Runtime inference using subprocess to avoid daemon limitations"""
        import subprocess
        import time
        ...
        # Get the path to the ONNX Runtime inference script
        script_path = os.path.join(os.path.dirname(__file__), 'run_nnunet_inference_fp16_onnx.py')

        # Prepare arguments for subprocess
        folds_str = ','.join(map(str, self.folds))
        device_str = 'cuda' if self.device.type == 'cuda' else 'cpu'

        # ONNX path (relative to script directory)
        onnx_path = os.path.join(os.path.dirname(__file__), 'nnunet_fp16.onnx')

        logger.info(f"   Running ONNX Runtime inference subprocess...")
        ...
        # Add --rebuild flag only if ONNX doesn't exist
        if not os.path.exists(onnx_path):
            cmd.append('--rebuild')
            logger.info(f"   ONNX file not found, will export on first run")
        else:
            logger.info(f"   Using cached ONNX file")

        # Prepare environment with CUDA paths
        env = os.environ.copy()
        env['PATH'] = '/usr/local/cuda-12.3/bin:' + env.get('PATH', '')
        env['LD_LIBRARY_PATH'] = '/usr/local/cuda-12.3/lib64:/usr/lib/wsl/lib:' + env.get('LD_LIBRARY_PATH', '')
        ...
        try:
            logger.info(f"   Starting ONNX Runtime subprocess (output will be shown in real-time)...")
            sys.stdout.flush()

            start_time = time.time()
            result = subprocess.run(cmd, env=env, timeout=3600)  # 1 hour timeout
            elapsed = time.time() - start_time

            if result.returncode != 0:
                error_msg = f"ONNX Runtime inference subprocess failed with return code {result.returncode}"
                logger.error(f"   {error_msg}")
                sys.stdout.flush()
                raise Exception(error_msg)
            logger.info(f"   ONNX Runtime subprocess finished with code 0 in {elapsed:.2f}s")
            sys.stdout.flush()

        except subprocess.TimeoutExpired:
            raise Exception("TensorRT inference subprocess timed out after 1 hour")
        except Exception as e:
            raise Exception(f"Failed to run TensorRT inference subprocess: {str(e)}")
        ...
        logger.info(f"   Prediction file size: {os.path.getsize(output_file)} bytes")
        try:
        ...
        logger.info(f"   TensorRT inference completed, output saved to: {output_file}")
        sys.stdout.flush()
